...
/**
 * @fileOverview Detects potential self-deception in claims across various verticals using a multi-dimensional risk model.
 *
...
import { aiDetectDeception } from '@/ai/genkit-instances';
...
export async function detectSelfDeception(input: DetectSelfDeceptionInput): Promise<DetectSelfDeceptionOutput> {
  return detectSelfDeceptionFlow(input);
}

const detectSelfDeceptionPrompt = aiDetectDeception.definePrompt({
  name: 'detectSelfDeceptionPrompt',
  input: {schema: DetectSelfDeceptionInputSchema},
  output: {schema: DetectSelfDeceptionOutputSchema},
  prompt: `You are a systems thinking psychologist who specializes in identifying cognitive biases and patterns of self-deception using a multi-dimensional risk model. Analyze the user's claim within the given verticals.

  Claim: {{{claim}}}
  Verticals: {{#each verticals}}{{{this}}}{{#unless @last}}, {{/unless}}{{/each}}
  ...
  3.  **diagnosis**: A list of key findings. For each finding, provide the **bias** (dimension name, e.g., 'Cognitive Bias'), the **diagnosis** (your one-sentence explanation), and a **riskScore** (a float between 0.0 and 1.0 for that specific dimension). You should provide a diagnosis for 3-5 of the most relevant dimensions.

  Your goal is to provide a clear, structured, and actionable diagnosis.`,
});

const detectSelfDeceptionFlow = aiDetectDeception.defineFlow(
  {
    name: 'detectSelfDeceptionFlow',
    inputSchema: DetectSelfDeceptionInputSchema,
    outputSchema: DetectSelfDeceptionOutputSchema,
  },
  async input => {
    const {output} = await detectSelfDeceptionPrompt(input);
    return output!;
  }
);
